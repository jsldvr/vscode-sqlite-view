...
<body>
  ...
  <main>
    ...
    <section id="sql" class="tab">
      <div class="split"><aside><h2>Query builder</h2><label>Table<select id="builder-table"></select></label><label>Columns<input id="builder-columns" value="*"></label><label>Where<input id="builder-where" placeholder="status = 'active'"></label><label>Order by<input id="builder-order"></label><label>Limit<input id="builder-limit" type="number" value="100"></label><button id="build-query">Build SQL</button></aside>
      <div class="query-area"><textarea id="sql-editor" spellcheck="false">SELECT name, type FROM sqlite_schema ORDER BY type, name;</textarea><div class="toolbar"><button id="run-sql" class="primary">Run SQL</button><button id="clear-sql">Clear</button><span>Ctrl+Enter to run</span></div><div id="query-results"></div></div></div>
    </section>
