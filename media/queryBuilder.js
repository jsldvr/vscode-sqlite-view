(function (root, factory) {
  ...
  function normalizeProjection(projection) {
    ...
    if (projection.kind !== 'star' && projection.alias !== undefined && projection.alias !== null) {
      const alias = String(projection.alias).trim();
      if (alias) {
  ...
  function setAlias(projections, key, alias) {
    return projections.map(item => projectionKey(item) === key ? normalizeProjection({ ...item, alias }) : normalizeProjection(item));
  }
  ...
  function projectionExpression(projection) {
    ...
    return normalized.alias ? `${expression} AS ${quoteIdentifier(normalized.alias)}` : expression;
  }
  ...
  function projectionLabel(projection) {
    ...
    return normalized.alias ? `${label} AS ${normalized.alias}` : label;
  }
